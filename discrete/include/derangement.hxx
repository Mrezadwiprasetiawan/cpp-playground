...
requires(std::integral<T> && !std::is_same_v<T, bool>) class Derangement {
  ...
  static const inline T      magic_number = static_cast<T>(std::log((3.0 * std::sqrt(5.0)) / 5.0) /                        log_phi);;  // depends on the machine architecture, but this just approximation for 1 operation building recursive caches of the derangement
